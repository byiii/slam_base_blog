...
void axisAngleToRotationMatrix(const double *axis, double angle, Eigen::Matrix3f &mat, bool normalized_p)
{
    float ux = static_cast<float>(axis[0]);
    float uy = static_cast<float>(axis[1]);
    float uz = static_cast<float>(axis[2]);
    if(!normalized_p)
    {
        float axis_length = std::sqrt(ux*ux+uy*uy+uz*uz);
        ux = ux/axis_length;
        uy = uy/axis_length;
        uz = uz/axis_length;
    }

    Eigen::Vector3f eigen_axis(ux, uy, uz);
    Eigen::AngleAxisf angle_axis(static_cast<float>(angle), eigen_axis);
    mat = angle_axis.matrix();
}


void rotationMatrixToAxisAngle(const Eigen::Matrix3f &mat, Eigen::Vector3f &axis, float &angle)
{
    Eigen::AngleAxisf angle_axis(mat);
    axis = angle_axis.axis();
    angle = angle_axis.angle();
}

void eulerAngleToQuaternion_ZYX(const double *eulerAngles, Eigen::Quaternionf q)
{
...
void quaternionToEulerAngle_XYZ(const Eigen::Quaternionf &q, Eigen::Vector3f &eulerAngles)
{
    float q0 = q.w();
    float q1 = q.x();
    float q2 = q.y();
    float q3 = q.z();
    float phi = atan2(2*(q0*q1+q2*q3), 1-2*(q1*q1+q2*q2));
    float theta = asin(2*(q0*q2-q3*q1));
    float psi = atan2(2*(q0*q3+q1*q2), 1-2*(q2*q2+q3*q3));
...
void rotationMatrixToEulerAngle_XYZ(const Eigen::Matrix3f &mat, Eigen::Vector3f &eulerAngles)
{
...
void eulerAnglesToRotationMatrix_XYZ(const Eigen::Vector3f &eulerAngles, Eigen::Matrix3f &mat)
{
    float psi = eulerAngles(0);
    float theta = eulerAngles(1);
    float phi = eulerAngles(2);
    float s_psi   = sinf(psi);   float c_psi   = cosf(psi);
    float s_theta = sinf(theta); float c_theta = cosf(theta);
    float s_phi   = sinf(phi);   float c_phi   = cosf(phi);

    mat(0,0) = c_theta*c_phi; mat(0,1) = s_psi*s_theta*c_phi-c_psi*s_phi; mat(0,2) = c_psi*s_theta*c_phi+s_psi*s_phi;
    mat(1,0) = c_theta*s_phi; mat(1,1) = s_psi*s_theta*s_phi+c_psi*c_phi; mat(1,2) = c_psi*s_theta*s_phi-s_psi*c_phi;
    mat(2,0) = -s_theta;      mat(2,1) = s_psi*c_theta;                   mat(2,2) = c_psi*c_theta;
}
